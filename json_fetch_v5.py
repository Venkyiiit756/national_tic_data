...
def format_showtime(utc_time_str):
    try:
        ...
        # Format as "X:MM AM/PM" (remove leading zeros from hour)
        hour = ist_time.strftime("%I").lstrip('0')  # Remove leading zero
        minute = ist_time.strftime("%M")
        ampm = ist_time.strftime("%p")
...
final_sheet.title = "DetailedData"

# New headers for the detailed combined data
final_headers = ["City", "Theater name", "Audi", "Showtime", "AvailableTickets", "TotalTickets", 
                "BookedTickets", "Occupancy (%)", "TotalGross", "BookedGross"]
...
# Set column widths for final sheet
column_widths = [15, 25, 15, 15, 18, 15, 15, 15, 15, 15]
...
for json_file_path in json_file_paths:
    ...
    # Define headers for individual city Excel files
    ...
    # Set column widths for individual city Excel sheets
    ...
    # Initialize data containers for city Excel
    sheet2_data = {}
    sheet3_data = {}
    ...
    for cinema in data.get("meta", {}).get("cinemas", []):
        ...
        for session in sessions:
            ...
            # Initialize aggregated data for this showtime
            showtime_data = {
                "AvailableTickets": 0,
                "TotalTickets": 0,
                "BookedTickets": 0,
            ...
            for area in session.get("areas", []):
                ...
                # Accumulate data for Sheet2 (city Excel)
                ...
                # Accumulate data for Sheet3 (city Excel)
                ...
                # Accumulate data for final combined Excel
                showtime_data["AvailableTickets"] += sAvailTickets
                showtime_data["TotalTickets"] += sTotalTickets
                showtime_data["BookedTickets"] += sBookedTickets
                showtime_data["TotalGross"] += sTotalGross
                showtime_data["BookedGross"] += sBookedGross

            # Calculate occupancy for this showtime
            occupancy = (showtime_data["BookedTickets"] / showtime_data["TotalTickets"]) if showtime_data["TotalTickets"] else 0

            # Add row to final combined Excel
            final_sheet.append([
                city_name,
                theater_name,
                audi,
                show_time,
                showtime_data["AvailableTickets"],
                showtime_data["TotalTickets"],
                showtime_data["BookedTickets"],
                occupancy,
                round(showtime_data["TotalGross"], 2),
                round(showtime_data["BookedGross"], 2)
            ])

            # Accumulate city data for consolidated sheet
            ...
            city_data[city_name]["AvailableTickets"] += showtime_data["AvailableTickets"]
            city_data[city_name]["TotalTickets"] += showtime_data["TotalTickets"]
            city_data[city_name]["BookedTickets"] += showtime_data["BookedTickets"]
            city_data[city_name]["TotalGross"] += showtime_data["TotalGross"]
            city_data[city_name]["BookedGross"] += showtime_data["BookedGross"]

# Write Sheet2 data (city Excel)
...
# Format final combined Excel sheet
...
for row in final_sheet.iter_rows(min_row=2, min_col=8, max_col=8):
    for cell in row:
        cell.number_format = '0.00%'

# Add total row to final sheet
add_total_row(final_sheet, 2, final_sheet.max_row, [5, 6, 7, 9, 10])

# Create consolidated city data sheet
consolidated_sheet = final_workbook.create_sheet(title="ConsolidatedCityData")
consolidated_headers = ["City", "AvailableTickets", "TotalTickets", "BookedTickets", 
                       "Occupancy (%)", "TotalGross", "BookedGross"]
...
# Format occupancy column as percentage in consolidated sheet
...
print("- Detailed show-time level data in combined Excel")
print("- Corrected IST time format")
